...
それらは supplemental または Projection に逃がします。
...
重要なのは、Projection が読むのは常に「テーブル」ではなく **宣言済み入力集合** だという点です。
...
source-native を読んだ場合は、lineage に `sourceNativeRead { system, timestamp, revisionIfKnown }` を記録する。revision が取れない場合はタイムスタンプのみで可。
